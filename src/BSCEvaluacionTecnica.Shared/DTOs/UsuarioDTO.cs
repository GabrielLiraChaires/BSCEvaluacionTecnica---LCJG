    ...
    public class UsuarioDTO : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            if (!Edicion)
            {
                ...
                {
                    yield return new ValidationResult("La contraseña es obligatoria.", new[] { nameof(Clave) });
                }
            }
            ...
            if (!string.IsNullOrWhiteSpace(Clave))
            {
                if (Clave.Length < 8)
                {
                    yield return new ValidationResult("La contraseña debe tener al menos 8 caracteres.", new[] { nameof(Clave) });
                }

                var regex = new Regex(@"^(?=.*[A-Z])(?=.*\d)[A-Za-z\d]+$");
                if (!regex.IsMatch(Clave))
                {
                    yield return new ValidationResult("La contraseña debe contener al menos una letra mayúscula, un número y no debe incluir caracteres especiales.", new[] { nameof(Clave) });
                }
            }
        }
    }
